...
(Figure~\ref{fig:grounding}). A plausible interpretation is that
uncertain predictions trigger more attention to the image as the model
searches for visual evidence, which runs counter to the ``high
grounding implies correct'' assumption that the six grounding gates
rely on.
...
The adversarial split is doing exactly what its design intends. This
structural property now has full inferential support: across 3,000
questions the attention, logit-gap, and CLIP-L signals all confirm that
co-occurring negatives are visually indistinguishable from true
positives at the feature level the grounding gates access.
...
Appendix~\ref{app:template} verifies that the dynamic readout is stable
across two prompt templates on LLaVA-1.6-Mistral-7B, the LLaVA-1.5 paper
template and the model's native chat template, on the first 500 POPE
adversarial questions, and that the legacy LLaVA-1.5 readouts collapse on
the Mistral vocabulary under both.
...
As of the present version, Denominator~1 (papers whose readout we can
verify) $=3$, Numerator (verified gains that shrink or reverse against
the eight-token baseline of 82.21) $=3$, and Denominator~2 (papers
surveyed in total) $=8$; the three verified gains are Tang et al.\
($+10.81 \to +2.14$), Seo et al.\ ($\to -2.21$), and Chen et al.\
($\to -0.41$). Appendix~\ref{app:survey} states the procedure for filling
the scaffold and the full definitions of the two denominators and the
numerator. Community contributions that expand both denominators are
welcome via pull requests against \texttt{references/paper\_survey.csv}.
...
\begin{enumerate}
\item Log the generated token id and decoded string for every
      question. A first-token argmax and its tokenizer-decoded
      surface form are cheap to record and remove the need to guess
      at the readout convention later.
\item Verify yes/no token sets under the exact prompt template.
      Different templates push first-token mass onto different
      surface forms. The
      \texttt{USER:~<image>\textbackslash n\{question\}\textbackslash nASSISTANT:}
...
\item Prefer the tokenizer-derived single-token logit readout as the
      default. Deriving the yes/no IDs from the model's own tokenizer
      and comparing their max-logits is deterministic and depends only
      on the logit vector, not on what text a generation loop returns.
      String parsing is equivalent (Theorem~\ref{thm:equiv}) when it
      reads exactly the first generated token; parsing a multi-token
      free-text span is not safe in general, as the InstructBLIP run in
      Section~\ref{sec:crossmodel} shows, where the span did not isolate
      the answer token and the readout collapsed to all-yes.
\item If using logits, include all tokenizer surface forms.
...
\item Release prediction-level records. A JSON file with per-question
      \texttt{question\_id}, \texttt{label}, prediction, and
      first-token id allows reviewers to recompute any aggregate
      metric and to verify the token-readout convention independent
      of summary numbers.
\item Report prompt template, tokenizer, model revision, and yes/no
      token ids. These four items determine whether a comparison
      across papers reads the same protocol or a different one; any
      reproducibility claim that omits them is partial.
\end{enumerate}
